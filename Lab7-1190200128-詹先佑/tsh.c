...
int builtin_cmd(char **argv) 
{
	...
	flag1 = strcmp(argv[0],"quit");//判断是否输入的是quit 
	flag2  =strcmp(argv[0],"fg");//判断是否输入的是fg 
    flag3  =strcmp(argv[0],"bg");//判断是否输入的是bg
    flag4  =strcmp(argv[0],"jobs");//判断是否输入的是jobs
    flag5 = strcmp(argv[0],"&");//判断是否输入的是&
	if(flag1 == 0)
	{
		exit(0); //如果输入的是quit则退出程序 
	} 
	if(flag2==0)
	{
		do_bgfg(argv) ;//如果输入的是fg则调用 do_bgfg函数实现相应的功能 
		return 1;
	}
	if(flag3==0)
	{
		do_bgfg(argv) ;//如果输入的是bg则调用 do_bgfg函数实现相应的功能 
		return 1;	
	}
	if(flag4==0)
	{
		listjobs(jobs) ;//如果输入的是jobs则调用 listjobs函数显示当前暂停的进程
		return 1;
	}
...
void waitfg(pid_t pid)
{
	while(pid==fgpid(jobs))//只要指定为pid的进程还在前台运行循环就继续 
	{
		sleep(1);//休眠1秒 
	}
    return;
...
void sigchld_handler(int sig) 
{
	...
	sigfillset(&mask_all);
	int status;
	while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) //只要有子进程被终止或者停止就回收这个进程 
	{
